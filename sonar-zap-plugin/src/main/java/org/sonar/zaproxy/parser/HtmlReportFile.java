...
public class HtmlReportFile {
  ...
  public HtmlReportFile(ZapSensorConfiguration configuration, FileSystem fileSystem, PathResolver pathResolver) {
   ...
   *
   * @throws org.sonar.api.utils.MessageException if the property relates to a
   *                                              directory or a non-existing
   *                                              file.
   */
  @CheckForNull
  private File getReportFromProperty() {
    ...
    if (report != null && !report.isFile()) {
      LOGGER.warn("ZAP HTML report does not exist. SKIPPING. Please check property "
          + ZapConstants.HTML_REPORT_PATH_PROPERTY + ": " + path);
      return null;
    }
    return report;
  }
